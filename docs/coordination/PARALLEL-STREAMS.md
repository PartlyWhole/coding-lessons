...
|---|---|---|---|
| speedup-suite · test-runtime | ✅ **integrated** @ `442e7c9` (2026-06-10, two-phase) | merged; worktree pruned | The rev-2 analysis implemented (plan: `docs/superpowers/plans/2026-06-10-trellis-speedup-suite.md`): **warm-server CPython twin** in `local-cpython.ts` (length-prefixed JSON protocol, fd-dup framing hygiene, host-side watchdog kill→respawn→`timedOut:true`, RSS-watermark recycle, orphan hygiene via stdin-EOF; `RUN_HARNESS` byte-untouched — server loop appended in the driver string) + **batched gate bridge** (`runCases` embedded driver, per-case SIGALRM isolation, classification lockstep with `run_case.py` test-pinned) + **two-phase gates 5/6 batching** (`signals.ts` collect/finish split, E-15 short-circuit verbatim; gate 5 ~100 spawns→2, gate 6 12→1) + **content-hash disk cache** (pure memo, `node_modules/.cache/trellis-gate-exec/`, `TRELLIS_GATE_CACHE=0` kill switch, cold==warm identity tested) + `parsePython` memo. **Headlines (re-baseline → final, solo):** cli 69.0s→1.5s · gate-fixtures 26.7s→1.3s · differential 72.4s→18.7s · m4 e2e 5.5s→0.6s · forced suite ~2-3m→~71s median (44.9s cold-cache). **Concurrency sweep: NO PIN** (median wins inside the ±noise band; decision rule held — root config untouched). Three sanctioned test-budget widenings (timeout-only, documented). `py/run_case.py` now runtime-dead (kept as reference impl). **Workspace = 672** (sandbox 83 · authoring 159). Merged battery green exit-0 (browser harnesses not in the touched path — twin/bridge are offline tooling); orchestrator re-verified cli solo 2.2s. Follow-ups queued: CI actions-cache for the gate-exec cache; harness.py 5s subprocess budget under load (flake headroom, build-owned); `ast_dump.py` batching. |
| E-18 · node vocabulary | ✅ **integrated** @ `c324d52` (2026-06-10, successor's first §4.1) | merged; worktree pruned | FunctionDef + Tuple + UnaryOp + BoolOp added to BOTH §6.3 NODE_TYPES tables (harness.py + sandbox ast-query.ts, byte-identical additive entries + lockstep comments; predecessor line-by-line review re-verified by the successor at merge). Tests only otherwise: 3-way differential (authoring TS == sandbox twin == harness.py) + non-vacuity + op-class out-of-vocabulary documentation + the protocol §6 row 9 `field`-in-`within` divergence pin. Rebased `abc970b`→`c324d52` (docs-only divergence). **Workspace = 649** (sandbox 75 · authoring 144). Merged battery green incl. `--browser` (ast-query is live worker code): m6 15/15 + m65 9/9 + crash-repro 4/4; evidence refreshed at merged HEAD. Protocol §6 row 8 flipped LANDED — content's mechanical proxy-tag upgrade unblocked. |
| (handshake) · content succession | 📋 **closed build-side** (2026-06-10) | n/a | Pre-succession handshake answered in full: `INTER-ORCHESTRATOR-PROTOCOL.md` **§7** (the successor's entry point, with §6 row 8 = the newly-accepted E-18 node vocabulary, in flight). E-15 finding of record now a design note: `docs/design-notes/2026-06-10-module-fault-ran-true.md`. Re-pin ownership re-confirmed (count will exceed 24 — re-measured at review; M6 surfaces also pin the old marquee). Rebuild content-complete on their branch: 23 nodes · 39 skills · 126 misconceptions @ `f466459`. |
| M6 · telemetry | ✅ **integrated** @ `7d55813` (2026-06-10; bounced once, re-landed) | merged; worktree pruned | `@trellis/telemetry` (§11 complete: bus+types moved in, client eventBus = re-export shim; Recorder sole behavioral_event writer, gap-free seq, no raw text; IdleDetector injected-clock; EventBuffer size/interval/pagehide; ProactiveScaffolder 4 rules HEADLESS — design ⚑ queued — except three_fail_streak via existing HintLadder) + 3 sanctioned emit sites (run/editor_change-debounced/focus, zero behavior change) + one-line attach. **BOUNCE STORY:** orchestrator's merged-result m65 re-run caught stacked pygame loops; fix session disproved the emit-threading hypothesis and root-caused a PRE-EXISTING §17.3 race (shared Pyodide globals re-bind GEN; kill-on-restart was a ~20ms phase race, 2/6 stacked at pre-M6 main — historical 9/9s were luck). Fix: fresh globals per start in `packages/runtime` → kill UNCONDITIONAL. **Workspace = 637** (telemetry 51 · client 100 · runtime 23). Battery green: m6 15/15 + m65 9/9 deterministic + crash-repro 4/4. |
| seam-asks · E-14/17/16/15 | ✅ **integrated** @ `99cba79` (2026-06-10) | merged; worktree pruned | All four content-track seam escalations landed, each TS+harness(+sandbox) halves in ONE commit: **E-14** Subscript+List node types (3-way lockstep; finding: no authoring whitelist exists — enforcement = the two NODE_TYPES tables + a new 3-way differential test) · **E-17** gate 10 graphical-step lint (pairing/entrypoints-exist/frames≤600+tape/preamble-locked; validate.py port clause-for-clause) · **E-16** §7 attribution-mode ported into TS gate 5 (detect-winner mirrored rank-by-rank from engine detect.ts + harness detect_winner; the content track's 9 pre-migration co-fire forms embedded as permanent goldens, harness subprocess differential 9/9 agreement) · **E-15** harness stops computing tests on watchdog-killed runs (mirrors evaluate.ts:39-52 bare-run short-circuit). ⚠️ **E-15 FINDING (relayed to content):** module-level runtime faults keep `ran:true` in the real RUN_HARNESS → engine DOES compute tests on them; the content track's withheld-fixture premise was wrong and their F7 `all:[astTag,testFailure]` gating would mis-attribute live. Also flagged: engine `evaluate.test.ts:187` mocks `ran:false` for module-level errors — disagrees with the engine's own harness (frozen, queued). **Workspace = 575** (sandbox 71 · authoring 136). Battery green via the new exit-code-strict `verification/run-all-gates.sh` incl. m65 9/9 (sandbox ast-query is live worker code) + crash-repro 4/4. |
