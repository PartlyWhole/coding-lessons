...
`main` @ `4e8eca5`. **M1 + M2 + M3a + M3b + M4 + M5-persist all integrated** (5 packages). Merged result
verified green: **350 tests** (schema 37 + engine 159 + authoring 80 + sandbox 55 + **persist 19**);
typecheck/lint/build clean; native-ESM imports of all 5 `dist` clean; content oracle PASS. The timedOut
re-key remains HELD for the real-Pyodide session (see E row). **Live front: Stream G (M5-client)** — both
its inputs (E + F) are now on `main`. Earlier checkpoints: M0–M3b core 288 tests; +M4 = 331; typecheck/lint/build clean; native-ESM `import()` of each `dist/src/index.js` OK (the new
...
| G · M5-client | 🆕 **launching** @ `4e8eca5` (E+F now both on `main`) | `../trellis-m5-client` / `m5-client` | NEW `@trellis/client` — React `CellRunner` (§5.2 keyed step replacement), step views, CodeMirror, attribution-colored feedback, hint panel (§9.2), peek-back (§5.3), `EventBus` stub (§11.1). Consumes engine (M2 step machine + M4 ladder) + persist + sandbox host. **Whole-slice acceptance gate is itself partly deferred** — the in-browser Pyodide walkthrough needs a networked browser; build + verify offline (component tests, wiring vs M3a mock + M3b local-CPython twin, reload via fake IDB). |
