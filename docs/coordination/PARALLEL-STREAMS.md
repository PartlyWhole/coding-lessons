...
|---|---|---|---|
| L · 🔥 client crash hotfix | 🚧 **LAUNCHED 2026-06-09** | `../trellis-l-hotfix` / `l-client-crash-fix` (off `6a33409`) | LIVE-SITE crash (user-reported): submit dispatched in FEEDBACK (double-click / Enter in mounted input) → fail-fast stepMachine throws inside the React reducer → tree unmounts; collateral IDB closing-race unhandled rejection from in-flight `persistDiagnosis`. Fix = guard the client dispatch boundary (race-safe phase guard in `useCellRunner` + view form/Enter paths) + teardown-safe persist. Engine machine FROZEN (its throw is correct). Owns `packages/client/**`. Browser-level double-submit regression required; walkthrough stays 8/8. Merge auto-deploys. |
| J · sandbox hardening | 🚧 **LAUNCHED 2026-06-09** | `../trellis-j-sandbox` / `j-sandbox-hardening` (off `4f8365e`) | Escalations 3+4: js-FFI unreachable from learner code (RUN_HARNESS/worker; host machinery intact) + honest `memoryMb` enforcement (visible failure + worker recycle; never a silent no-op). Owns `packages/sandbox/**` + `verification/**`. Evidence bar: debt1 checks 5+11 flipped to PASS in a real browser; run-debt5 stays 8/8; differential green. Must not break I′'s conditional exports / browser entry. |
| K · authoring lint | 🚧 **LAUNCHED 2026-06-09** | `../trellis-k-authoring` / `k-authoring-lint` (off `4f8365e`) | Escalation 6a: gates for answerable choice-mode predicts (mirror `diagnose.ts` `matchesAccepted`) + no misconception tag on a correct choice; mutation-tested against the two real historical bugs (`0884bf3`, `d8f20b3`). Owns `packages/authoring/**`. Corpus must stay PASS; gate-failure on real content = escalate, not edit. |
