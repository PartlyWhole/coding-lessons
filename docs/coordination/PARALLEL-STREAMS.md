...
|---|---|---|---|
| H · evaluate routing | 🚧 **LAUNCHED 2026-06-09** | `../trellis-h-evaluate` / `h-evaluate-routing` (off `e30348e`) | Escalation 1 (route `!ran`/`timedOut` through `detect()`) + escalation-5 follow-through (`canonicalDiagnosis`, byte-identity modulo wallMs per `docs/design-notes/2026-06-09-wallms-determinism.md`). Owns `packages/engine/**`. |
| I′ · client host + Greenhouse | 🚧 **LAUNCHED 2026-06-09** | `../trellis-i-client` / `i-client-host-greenhouse` (off `e30348e`) | Escalation 2 (real esbuild static host, browser-safe sandbox entry, workerUrl, Fontsource) + the approved Greenhouse integration per `docs/design/2026-06-09-design-handback.md` (drop-in tokens/ui/editor-theme, ⚑ markup additions, attribution hexes). Owns `packages/client/**` + the narrow sandbox packaging seam; sanctioned to delete `verification/app/`. Gate: 42 tests + marquee + real-browser walkthrough of the BUILT app. |
