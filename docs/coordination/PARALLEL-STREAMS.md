...
|---|---|---|---|
| E-18 · node vocabulary | 📦 **STAGED** — successor's first §4.1 (2026-06-10) | `../trellis-e18` / `e18-node-vocab` @ `abc970b` | Built + predecessor-reviewed (line-by-line; byte-identical lockstep table additions + tests only; stream gates 649 green foreground; 3-way differential 16×12 from the content track's gated tags; op-classes declared out-of-vocabulary, documented in-table). Deliberately NOT merged — clean-cut succession choice. Handoff §2 item 0 is the integration runbook. Bonus finding pinned on the branch: protocol §6 row 9 (within/field 3-way divergence, pre-existing). |
