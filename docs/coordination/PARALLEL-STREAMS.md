...
`main` @ `cf7daa7`. **M1 + M2 + M3a + M3b + M4 all integrated.** Merged result verified green:
**331 tests** (schema 37 + engine 159 + authoring 80 + sandbox 55) + the gate-7 golden + the timedOut
re-key HELD for the real-Pyodide session (see E row). Earlier checkpoint (M0–M3b core) was 288 tests:
(schema 37 + engine 120 + authoring 76 + sandbox 55); typecheck/lint/build clean; native-ESM `import()` of each `dist/src/index.js` OK (the new
