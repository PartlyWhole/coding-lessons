...
| 8 | **E-18 node vocabulary: FunctionDef, Tuple, UnaryOp, BoolOp** (surfaced in the 2026-06-10 handshake — never previously filed) | corpus carries comment-marked gated tags awaiting these | **ACCEPTED + IN FLIGHT** — same 3-way-lockstep pattern as E-14 (harness.py + sandbox ast-query.ts + differential tests); lands BEFORE the handover review so the mechanical upgrade isn't blocked. **Fact for the record: `99cba79` covered ONLY Subscript+List (E-14)** — the E-18 four are in neither table until this lands |
