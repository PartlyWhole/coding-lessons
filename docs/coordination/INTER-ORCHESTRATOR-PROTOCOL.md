...
| 8 | **E-18 node vocabulary: FunctionDef, Tuple, UnaryOp, BoolOp** (surfaced in the 2026-06-10 handshake — never previously filed) | corpus carries comment-marked gated tags awaiting these | ✅ **LANDED @ `c324d52`** (2026-06-10, successor build orchestrator's first §4.1; pushed to `main`) — both §6.3 NODE_TYPES tables (harness.py + sandbox ast-query.ts), byte-identical additive entries; 3-way differential green at the merged HEAD (workspace 649, full battery incl. browser harnesses). **GREEN LIGHT for your mechanical proxy-tag upgrade** after rebasing onto this `main`. Field reality documented in-table: FunctionDef.name via `where {attr:name}` not `field`; UnaryOp/BoolOp op-classes OUT of vocabulary (your boolop tag is op-agnostic so nothing blocks). Row 9 (`field`-in-`within` divergence) remains OPEN — its pinning test now lives on `main` |
