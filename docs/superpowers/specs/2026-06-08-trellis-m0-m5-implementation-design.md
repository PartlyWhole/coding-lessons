...
**In scope:** the complete deterministic machine running on the proving slice — Output → Variables spine plus the string-concatenation extension — minus telemetry, the pygame graphical runtime, and offline/a11y hardening. This corresponds to design milestones **M0 through M5**.

**The slice's end state (M5 acceptance):** a real learner walks Output → Variables → unlocks String-Concat (a genuine multi-prerequisite gate) → hits the `implicit_coercion` misconception → climbs the learner-dosed hint ladder → passes, with mastery surviving a page reload, served from `python -m http.server` with zero server logic.
...
- **Content authored:** the Output and Variables spine nodes + the string-concat extension (§16's slice), in YAML per §13.1.
- **Gate:** the proving-slice content compiles to a bundle; a deliberately-cyclic fixture **fails** the build with the offending path reported; a dangling-id fixture fails; the multi-prereq extension edge validates.
...
- **Gate:** the multi-prereq gating diff — with `string.literal` mastered but `var.assign` not, the extension stays `locked`; mastering both flips it to `available` (the availability *diff*). Non-build attribution precedence is unit-tested (`pass`/`misconception`/`mismatch`). Same `(step, signals)` → identical Diagnosis, always.
